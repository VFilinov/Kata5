...
parser.add_argument('-log-nn-params', help='Log neuralnet params', required=False, action='store_true')
...
def main(args):
    ...
    model, swa_model, other_state_dict = load_model(checkpoint_file, use_swa, device="cpu", verbose=log_nn_params)
    ...
    if version <= 12 or (version >= 101 and version <= 199):
    ...
    if version >= 15 and version <= modelconfigs.max_base_version:
    ...
    def write_trunk(name,model):
        ...
        if model.metadata_encoder is not None:
            assert version >= 15 and version <= modelconfigs.max_base_version
    ...
    def write_policy_head(name,policyhead):
        ...
        if version <= 11 or (version >= 101 and version <= 199):
    ...
    logging.info(str(datetime.datetime.now(datetime.UTC)) + " UTC")
