...
if __name__ == "__main__":
    ...
    optional_args.add_argument('-log-nn-params', help='Log neuralnet params', required=False, action='store_true')
    optional_args.add_argument('-model-version', help='Override model version', type=int, required=False)
...
def make_dirs(args):
    ...
    statdir =  os.path.join(traindir,"../../lstat")
    if not os.path.exists(statdir):
      os.makedirs(statdir)

    #filegpu = os.path.join(traindir,"multigpu")
    #if os.path.exists(filegpu):
    #    os.remove(filegpu)
...
def main(rank: int, world_size: int, args, multi_gpu_device_ids, readpipes, writepipes, barrier):
    ...
    statdir =  os.path.join(traindir,"../../lstat")
    #filegpu = os.path.join(traindir,"multigpu")
    epochjsonpath = os.path.join(traindir,"model_epoch.json")
    text_epoch = os.path.join(statdir,"epoch.txt")
    ...
    log_nn_params = args["log_nn_params"]
    ...
    log_list = ["loss", "p0loss", "p1loss", "vloss", "nsamp", "time_since_last_print"]
    #log_list = None

    if model_version is None:
       model_version = modelconfigs.default_version
    if lr_scale is None:
        lr_scale = 1.0
    if lr_scale_auto:
    ...
    if torch.cuda.is_available():
        ...
        device_type = "cuda"
        device = torch.device(device_type, my_gpu_id)
        logging.info("Using GPU device: " + str(my_gpu_id) + ": " + torch.cuda.get_device_name(device))
    else:
    ...
    def wait(rank, barrier, op):
        if barrier is not None:
          #logging.info(f"Before barrier {op} with rank {rank}")
          barrier.wait()
          #logging.info(f"After barrier {op} with rank {rank}")
    ...
    while True:
        ...
        logging.info("BEGINNING NEXT EPOCH " + str(train_state["export_cycle_counter"]))
        logging.info("=========================================================================")
        ...
        for i in range(sub_epochs):
            ...
            if rank == 0:
                ...
                wait(rank, barrier,"send train_files_to_use")
                ...
                wait(rank, barrier,"read train_files_to_use")
            ...
            wait(rank, barrier,"Beginning training subepoch")
            ...
            for batch in data_processing_pytorch.read_npz_training_data(
                ...
                if batch_count_this_epoch % print_train_loss_every_batches == 0:
                    ...
                    log_metrics(running_metrics["sums"], running_metrics["weights"], metrics, train_metrics_out, log_list)
        ...
        if rank == 0:
            ...
            if len(val_files) == 0:
                ...
                with torch.no_grad():
                    ...
                    log_metrics(val_metric_sums, val_metric_weights, metrics, val_metrics_out, log_list)
                    t1 = time.perf_counter()
        ...
        if rank == 0:
            ...
            with open(epochjsonpath,"w") as f:
              dataepoch = {"epoch": train_state["export_cycle_counter"]}
              json.dump(dataepoch, f)
              f.flush()

            with open(text_epoch,"w") as f:
              f.write(str(train_state["export_cycle_counter"]))
              f.close()
