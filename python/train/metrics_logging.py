...
def log_metrics(metric_sums, metric_weights, metrics, metrics_out, log_list=None):
    metrics_to_print = {}
    ...
    logging.info(", ".join(["%s = %f" % (metric, metrics_to_print[metric]) for metric in metrics_to_print if log_list is None or metric.startswith(tuple(log_list))]))
    if metrics_out:
