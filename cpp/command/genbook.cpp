  ...
  if(bonusFile != "") {
    ...
    sgf->iterAllPositions(
      ...
      [&](Sgf::PositionSample& unusedSample, const BoardHistory& sgfHist, const string& comments) {
        (void)unusedSample;
        if(
          comments.size() > 0 && (comments.find("BONUS") != string::npos || comments.find("EXPAND") != string::npos ||
                                  comments.find("VISITS") != string::npos || comments.find("BRANCH") != string::npos)) {
            ...
            if(comments.find(commandName) != string::npos) {
              ...
              try {
                vector<string> nextWords =
                  Global::split(Global::trim(comments.substr(comments.find(commandName) + std::strlen(commandName))));
                if(nextWords.size() <= 0)
          ...
          if(parseCommand("BONUS", ret)) {
            ...
            for(int bookVersion = 1; bookVersion <= Book::LATEST_BOOK_VERSION; bookVersion++) {
              ...
              logger.write(
                "Adding bonus " + Global::doubleToString(ret * bonusFileScale) + " to hash " + hashRet.toString());
            }
          }
          ...
          if(parseCommand("EXPAND", ret)) {
            ...
            for(int bookVersion = 1; bookVersion <= Book::LATEST_BOOK_VERSION; bookVersion++) {
              ...
              logger.write(
                "Adding expand bonus " + Global::doubleToString(ret * bonusFileScale) + " to hash " +
                hashRet.toString());
            }
          }
          ...
          if(parseCommand("VISITS", ret)) {
            ...
            for(int bookVersion = 1; bookVersion <= Book::LATEST_BOOK_VERSION; bookVersion++) {
              ...
              logger.write(
                "Adding required visits " + Global::doubleToString(ret * bonusFileScale) + " to hash " +
                hashRet.toString());
            }
          }
          ...
          if(parseCommand("BRANCH", ret)) {
            ...
            for(int bookVersion = 1; bookVersion <= Book::LATEST_BOOK_VERSION; bookVersion++) {
              ...
              logger.write(
                "Adding required branching factor " + Global::intToString((int)ret) + " to hash " + hashRet.toString());
            }
          }
        }
      });
...
int MainCmds::genbook(const vector<string>& args) {
  ...
  if(bookFileExists) {
    ...
    if(!allowChangingBookParams) {
      ...
      if(
        ...
        // cfgParams.adjustedVisitsWLScale != existingBookParams.adjustedVisitsWLScale || // not used
  ...
  auto setNodeThisValuesFromFinishedSearch = [&](
    ...
    double errors = search->getShallowAverageShorttermWLError(searchNode);
  ...
  if(traceBook != NULL || traceSgfFile.size() > 0) {
    ...
    for(int iteration = 0; iteration < numIterations; iteration++) {
      ...
      std::vector<SymBookNode> nodesToExpand =
        book->getNextNToExpand(std::min(1 + iteration * 2 + (iteration * iteration / 25), numToExpandPerIteration));
...
int MainCmds::writebook(const vector<string>& args) {
  ...
  const int64_t maxVisitsForLeaves = cfg.contains("maxVisitsForLeaves")
                                       ? cfg.getInt64("maxVisitsForLeaves", (int64_t)1, (int64_t)1 << 50)
                                       : (params.maxVisits + 1) / 2;
