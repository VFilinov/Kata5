...
int MainCmds::evalsgf(const vector<string>& args) {
  ...
  //Parse rules -------------------------------------------------------------------
  Rules defaultRules = Setup::loadSingleRules(cfg);
  ...
  auto setUpBoardUsingRules = [&board, &nextPla, &hist /*, overrideKomi*/, &sgf, &extraMoves](
                                const Rules& initialRules, int moveNum) {
  ...
  for(int moveNum = moveNumStart; moveNum <= moveNumEnd; moveNum++) {
    ...
    //     NNOutput* nnOutput = buf.result.get();
    //     cout << nnOutput->whiteWinProb << " " << nnOutput->shorttermWinlossError << endl;
    //   }
    //   continue;
    // }
    ...
    if(rawNN) {
      ...
      if(humanEval != NULL) {
        humanEval->evaluate(board, hist, nextPla, nnInputParams, buf, skipCache);
        buf.result->debugPrint(cout, board);
      }
      continue;
    }
    ...
    if(options.branch_.size() > 0) {
      ...
      for(int i = 0; i < options.branch_.size(); i++) {
        ...
        if(!copyHist.isLegal(copy, loc, pla)) {
          cerr << board << endl;
          cerr << "Branch Illegal move for " << PlayerIO::colorToChar(pla) << ": " << Location::toString(loc, board)
               << endl;
          return 1;
        }
...
#else
      auto doPrintPolicy = [&](const NNOutput* nnOutput, int nnXLen, int nnYLen) {
        for(int y = 0; y < board.y_size; y++) {
          for(int x = 0; x < board.x_size; x++) {
            int pos = NNPos::xyToPos(x, y, nnXLen);
        ...
        double prob = nnOutput->getPolicyProbMaybeNoised(
          NNPos::locToPos(Board::PASS_LOC, board.x_size, nnOutput->nnXLen, nnOutput->nnYLen));
        ;
        cout << "Pass " << Global::strprintf("%5.2f", prob * 100) << endl;
      };
#endif
...
#else
        doPrintPolicy(nnOutput, nnOutput->nnXLen, nnOutput->nnYLen);
#endif
      }
...
#else
        doPrintPolicy(humanOutput, humanOutput->nnXLen, humanOutput->nnYLen);
#endif
      }
    }
    if(printLogPolicy) {
...
#else
      auto doPrintLogPolicy = [&](const NNOutput* nnOutput, int nnXLen, int nnYLen) {
        for(int y = 0; y < board.y_size; y++) {
          for(int x = 0; x < board.x_size; x++) {
            int pos = NNPos::xyToPos(x, y, nnXLen);
            double prob = nnOutput->getPolicyProbMaybeNoised(pos);
...
#else
        doPrintLogPolicy(nnOutput, nnOutput->nnXLen, nnOutput->nnYLen);
#endif
      }
      const NNOutput* humanOutput = search->rootNode->getHumanOutput();
      if(humanOutput != NULL) {
...
#else
        doPrintLogPolicy(humanOutput, humanOutput->nnXLen, humanOutput->nnYLen);
#endif
      }
    }
    ...
    if(dumpNpzInputTo != "") {
      ...
      NNInputs::fillRowV7(
        board, hist, nextPla, nnInputParams, nnXLen, nnYLen, inputsUseNHWC, binaryInputNCHW.data, globalInputNC.data);
