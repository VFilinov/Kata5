...
struct GTPEngine {
  ...
  void setOrResetBoardSize(ConfigParser& cfg, Logger& logger, Rand& seedRand, int boardXSize, int boardYSize, bool loggingToStderr) {
    ...
    if(nnEval == NULL || !(nnXLen == nnEval->getNNXLen() && nnYLen == nnEval->getNNYLen())) {
      ...
      nnEval = Setup::initializeNNEvaluator(
        ...
        Setup::SETUP_FOR_GTP);
      logger.write(
        "Loaded neural net with nnXLen " + Global::intToString(nnEval->getNNXLen()) + " nnYLen " + Global::intToString(nnEval->getNNYLen()));
      if(humanModelFile != "") {
  ...
  void handleGenMoveResult(
    ...
    bool resigned = gargs.allowResignation && shouldResign(
      search->getRootBoard(),search->getRootHist(),pla,recentWinLossValues,
      gargs.resignThreshold,gargs.resignConsecTurns,gargs.resignMinMovesPerBoardArea
    );
      ...
      if(whichSymmetry == NNInputs::SYMMETRY_ALL || whichSymmetry == symmetry) {
        {
          ...
          NNResultBuf buf;
          bool skipCache = true;
          nnEval->evaluate(board,hist,pla,&analysisParams.humanSLProfile,nnInputParams,buf,skipCache);
        ...
        if(prevLoc != Board::NULL_LOC) {
          ...
          NNResultBuf buf;
          bool skipCache = true;
          nnEval->evaluate(prevBoard,prevHist,prevPla,&analysisParams.humanSLProfile,nnInputParams,buf,skipCache);
  ...
  string rawNN(int whichSymmetry, double policyOptimism, bool useHumanModel) {
    ...
    for(int symmetry = 0; symmetry < SymmetryHelpers::NUM_SYMMETRIES; symmetry++) {
      if(whichSymmetry == NNInputs::SYMMETRY_ALL || whichSymmetry == symmetry) {
        ...
        NNResultBuf buf;
        bool skipCache = true;
        nnEvalToUse->evaluate(board,hist,nextPla,&analysisParams.humanSLProfile,nnInputParams,buf,skipCache);
...
int MainCmds::gtp(const vector<string>& args) {
  ...
  const bool logAllGTPCommunication = cfg.contains("logAllGTPCommunication")?cfg.getBool("logAllGTPCommunication"):true;
  ...
  if(startupPrintMessageToStderr) {
  ...
  while(no_readline || getline(cin, line)) {
    no_readline = false;
    ...
    auto fnd_param = short_params.find(command);
    auto fnd_command = short_commands.find(command);
    ...
    if(command == "protocol_version") {
     ...
     engine->clearBoard();
      ...
      if(pieces.size() <= 0) {
        ...
        if(hasId)
          line = Global::intToString(id) + " " + Global::trim(pieces[0]);
        else
          line = Global::trim(pieces[0]);
        no_readline = true;
        continue;
      }
    }
      ...
      if(pieces.size() != 1 || (!Global::tryStringToInt(pieces[0], tmp))) {
        ...
        if(parseSuccess) {
          ...
          if(!suc) {
            ...
            string success = "Changed rules to " + newRules.toString();
    ...
    else if(Rules::basicRuleStrings().count(Global::toUpper(command))) {
      // Is Command a basic rule?
      if(pieces.size() != 0) {
        responseIsError = true;
    ...
    else if(Rules::VCNRuleStrings().count(Global::toUpper(command))) {
      // Is Command a VCN rule?
      if(pieces.size() != 0) {
        responseIsError = true;
    ...
    else if(fnd_param != short_params.end()) {
      // short record command
      if(pieces.size() > 1) {
        responseIsError = true;
        response =
          "Expected one or two arguments for " + fnd_param->first + " but got '" + Global::concat(pieces, " ") + "' ";
      } else {
        if(pieces.size() == 0) {
          line = "kata-get-param " + fnd_param->second;
        } else {
          line = "kata-set-param " + fnd_param->second + " " + pieces[0];
        }
        if(hasId)
          line = Global::intToString(id) + " " + line;

        no_readline = true;
        continue;
      }
    } else if(fnd_command != short_commands.end()) {
      // short record command
      if(pieces.size() > 1) {
        responseIsError = true;
        response =
          "Expected one or two arguments for " + fnd_command->first + " but got '" + Global::concat(pieces, " ") + "' ";
      } else {
          if(pieces.size() == 0) {
            line = fnd_command->second;
          } else {
            line = fnd_command->second + " " + pieces[0];
          }
        if(hasId)
          line = Global::intToString(id) + " " + line;
