  ...
  auto trySgf = [&](Sgf* sgf) {
    ...
    if(valueFluctuationNNEval == NULL) {
      ...
      for(size_t m = 0; m<sgfMoves.size()+1; m++) {
        ...
        bool skipCache = true;
        lock.unlock();
        valueFluctuationNNEval->evaluate(board, hist, nextPla, nnInputParams, buf, skipCache);
        lock.lock();
        ...
        if(sgfMoves[m].pla != nextPla && m > 0) {
          logger.write(
            "Ending SGF " + fileName + " early due to non-alternating players on turn " + Global::intToString(m));
          break;
        }
        ...
        if(!suc) {
          ...
          if(!suc)
            logger.write(
              "Illegal move in " + fileName + " turn " + Global::intToString(m) + " move " +
              Location::toString(sgfMoves[m].loc, board.x_size, board.y_size));
          break;
        }
      ...
      if(debugValueFluctuation) {
        ...
        WriteSgf::writeSgf(out, "B", "W", hists[hists.size() - 1], extraComments);
      ...
      for(size_t m = 0; m<hists.size(); m++) {
        blockedSituationHashes.insert(BoardHistory::getSituationRulesHash(
          hists[m].getRecentBoard(0), hists[m], hists[m].presumedNextMovePla));
      }
        ...
        [&blockedSituationHashes, &desiredWeight, &posHandler, trainingWeight](
          ...
          if(contains(
               ...
               BoardHistory::getSituationRulesHash(
                 posHist.getRecentBoard(0), posHist, posHist.presumedNextMovePla)))
            return;
          // cout << "BBBB" << endl;
...
int MainCmds::dataminesgfs(const vector<string>& args) {
 ...
 auto expensiveEvaluateMove = [&posWriter,&turnWeightLambda,&numFilteredIndivdualPoses,&surpriseMode,&minHintWeight,&hintScale,&logger,trainingWeight,moreUtilityWeight,utilityThreshold,minTurn](
    ...
    //Bot doesn't see the move?
      ...
      if(moreUtilityWeight > 0.0) {
        selfUtilityOfBestOtherMove =
          std::max(selfUtilityOfBestOtherMove, (nextPla == P_WHITE ? moveValues.utility : -moveValues.utility));
        selfUtilityOfHintMove =
          std::max(selfUtilityOfHintMove, (nextPla == P_WHITE ? missedValues.utility : -missedValues.utility));
      }
 ...
 // ---------------------------------------------------------------------------------------------------
 // GAME MODE
  auto processSgfGame = [&posWriter,&logger,&gameInit,&nnEval,&expensiveEvaluateMove,&gameModeFastThreshold,&maxDepth,&numFilteredSgfs,&maxPolicy,allowGameOver,manualHintOnly,trainingWeight,startPosesBeforeHintsLen,minTurn](
    ...
    for(int m = 0; m < moves.size(); m++) {
      ...
      double wroteHintPosWeight =
         expensiveEvaluateMove(search, moves[m].loc, nextPlas[m], boards[m], hists[m], sample, false, false);
  ...
  auto treePosHandler = [&gameInit,&nnEval,&expensiveEvaluateMove,&maxPolicy,/*&flipIfPassOrWFirst,*/&surpriseMode,trainingWeight](
    ...
    expensiveEvaluateMove(
      search, sample.hintLoc, pla, board, hist, sample, markedAsHintPos, markedAsHintPosLight);
  };
...
int MainCmds::checksgfhintpolicy(const vector<string>& args) {
  ...
  for(size_t i = 0; i < sgfFiles.size(); i++) {
    ...
    const std::vector<Rules> rulesToUse = {Rules::parseRules("chinese"), Rules::parseRules("japanese")};
      ...
      [&](Sgf::PositionSample& posSample, const BoardHistory& hist, const string& comments) {
        ...
        for(const Rules& rules: rulesToUse) {
          ...
          for(int symmetry = 0; symmetry < 8; symmetry++) {
            ...
            NNResultBuf buf;
            bool skipCache = true;
            nnEval->evaluate(board, histBefore, nextPla, nnInputParams, buf, skipCache);

            shared_ptr<NNOutput> nnOutput = std::move(buf.result);
            int pos = NNPos::locToPos(
              posSample.moves[posSample.moves.size() - 1].loc, board.x_size, nnOutput->nnXLen, nnOutput->nnYLen);
#ifndef QUANTIZED_OUTPUT
            double policy = nnOutput->policyProbs[pos];
#else
            double policy = nnOutput->getPolicyProbMaybeNoised(pos);
#endif
...
int MainCmds::evalrandominits(const vector<string>& args) {
  ...
  while(true) {
    ...
    cout << numInitialMovesToPlay << "," << values.winLossValue << endl;
  }
  delete evalBot;
  if(nnEval != NULL)
    delete nnEval;
