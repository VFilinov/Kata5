...
static void getNNEval(
  ...
  Board copy(board);
  nnEval->evaluate(copy,hist,nextPla,nnInputParams,buf,skipCache);
}
...
// ) {
//   NNResultBuf buf;
//   getNNEval(board,hist,nextPla,noResultUtilityForWhite,playoutDoublingAdvantagePla,playoutDoublingAdvantage,1000,nnEval,buf);
...
static ValueTargets makeWhiteValueTarget(
  ...
  if(hist.isGameFinished) {
    if(hist.isNoResult) {
      ...
      targets.win = (float)ScoreValue::whiteWinsOfWinner(copyHist.winner, noResultUtilityForWhite);
  ...
  NNResultBuf buf;
  int maxHistory = 1000;
  getNNEval(board,hist,nextPla,noResultUtilityForWhite,playoutDoublingAdvantagePla,playoutDoublingAdvantage,nnEval,buf);
...
int MainCmds::writetrainingdata(const vector<string>& args) {
  ...
  auto processSgf = [&](int threadIdx, size_t index) {
    ...
    const double noResultUtilityForWhite = 0.5;
    ...
    for(size_t m = startGameAt; m<sgfMoves.size()+1; m++) {
      ...
      ValueTargets targets = makeWhiteValueTarget(
        board, hist, nextPla, noResultUtilityForWhite, playoutDoublingAdvantagePla, playoutDoublingAdvantage, nnEval
      );
