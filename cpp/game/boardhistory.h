...
struct BoardHistory {
  ...
  std::string getMoves(int x_size, int y_size) const;
  ...
  bool fullBoard(int restMoves = 10, int depth = 0) const;
  bool maybePassMove(Player pla, int depth = 0, int restMoves = 10) const;

  //Compute a hash that takes into account the full situation, the rules, discretized komi, and any immediate ko prohibitions.
  static Hash128 getSituationRulesHash(const Board& board, const BoardHistory& hist, Player nextPlayer);


  private:
     // pass hash
     Hash128 getPassnumHash() const;
