...
string Location::toStringPsq(Loc loc, int x_size, int y_size, int coordinates) {
  int x = getX(loc, x_size);
  int y = getY(loc, x_size);

  bool coordLetters = (coordinates & 1);
  bool coordHJ = (coordinates & 2);
  bool flipHoriz = (coordinates & 4);
  bool flipVert = (coordinates & 8);
  bool flipDiag = (coordinates & 16);

  if(flipHoriz)
    x = (flipDiag ? y_size : x_size) - x - 1;
  if(flipVert)
    y = (flipDiag ? x_size : y_size) - y - 1;
  if(flipDiag) {
    int tmp = x;
    x = y;
    y = tmp;
  }
  loc = getLoc(x, y, x_size);

  if(x_size > 25 * 25 || !coordLetters)
    return toStringMach(loc, x_size);
  ...
  if(loc == Board::NULL_LOC)
    return string("null");
  const char* xChar = NULL;
  if(coordHJ) {
    xChar = "ABCDEFGHJKLMNOPQRSTUVWXYZ";
  }
  else {
    xChar = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  }
  if(x >= x_size || x < 0 || y < 0 || y >= y_size)
    return toStringMach(loc, x_size);
...
string Location::toStringPsq(Loc loc, const Board& b, int coordinates) {
  return toStringPsq(loc, b.x_size, b.y_size, coordinates);
}
...
void Board::printBoard(ostream& out, const Board& board, Loc markLoc, const vector<Move>* hist) {
  ...
  if(hist != NULL) {
    for(size_t i = 0; i < hist->size(); i++) {
      Loc loc = (*hist)[i].loc;
      out << Location::toString(loc, board.x_size, board.y_size);
    }
    out << "\n";
  }
}
