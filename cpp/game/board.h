...
namespace Location
{
  ...
  std::string toStringPsq(Loc loc, int x_size, int y_size, int coordinates);
  std::string toStringPsq(Loc loc, const Board& b, int coordinates);
...
struct Board
{
  ...
  static constexpr int DEFAULT_LEN = std::min(MAX_LEN,15); //Default edge length for board if unspecified
