...
struct Search {
  ...
  double getShallowAverageShorttermWLError(const SearchNode* node = NULL) const;
  ...
  bool getAnalysisJson(
    ...
    bool includePVVisits,  nlohmann::json& ret
  ) const;
...
private:
  ...
  AnalysisData getAnalysisDataOfSingleChild(
    ...
    Loc move, double policyProb, double fpuValue, double parentUtility, double parentWinLossValue, int maxPVDepth
  ) const;
  ...
  void getShallowAverageShorttermWLErrorHelper(
    const SearchNode* node,
