...
void TimeControls::getTime(const Board& board, const BoardHistory& hist, double lagBuffer, double& minTime, double& recommendedTime, double& maxTime) const {
  ...
  if(inTimePerMove && maxTimePerMove > 0 && maxTimePerMove < TimeControls::UNLIMITED_TIME_DEFAULT_LARGE) {
    minTime = 0;
    maxTime = maxTimePerMove;
    maxTime = applyLagBuffer(maxTime, lagBuffer);
    //recommendedTime = maxTimePerMove *2 / 3;
    recommendedTime = maxTimePerMove;
