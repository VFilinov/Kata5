...
void Search::setKomiIfNew(float newKomi) {
  if(rootHistory.rules.komi != newKomi) {
    ...
    searchParams.noResultUtilityForWhite = newKomi / 10.0;
  }
}
...
void Search::runWholeSearch(
  ...
  std::atomic<double> tcMaxTime(TimeControls::UNLIMITED_TIME_DEFAULT);
  std::atomic<double> upperBoundVisitsLeftDueToTime(TimeControls::UNLIMITED_TIME_DEFAULT);  
  const bool hasMaxTime = maxTime < TimeControls::UNLIMITED_TIME_MAX;
  ...
  if(!pondering && (hasTc || hasMaxTime)) {
    ...
    double tcLimit = TimeControls::UNLIMITED_TIME_MAX;
    if(hasTc) {
      ...
      vector<SearchNode*> filteredNodes;
      {
        ...
        for(; i<childrenCapacity; i++) {
          ...
          if(rootHistory.isLegal(rootBoard, moveLoc, rootPla) && isAllowedRootMove(moveLoc)) {
            if(moveLoc == Board::PASS_LOC && searchParams.suppressPass && !rootHistory.maybePassMove(rootPla)) {
              anyFiltered = true;
...
/*
  ...
  if(searchParams.useGraphSearch)
    rootGraphHash = GraphHash::getGraphHashFromScratch(rootHistory, rootPla);
  else
...
bool Search::playoutDescend(
  ...
  // we presumably want to actually explore deeper and get a result. Also it includes the node following a pass from the
  // root in
  // the case where we are conservativePass and the game would be ended. For friendlyPassOk rules, it may include deeper
  // nodes. Note that we also carefully clear the search when a pass from the root would be terminal, so nodes should
  ...
  if(thread.history.isGameFinished && !node.forceNonTerminal) {
    ...
    if(thread.history.isNoResult) {
      ...
      //double winLossValue = 2.0 * ScoreValue::whiteWinsOfWinner(thread.history.winner, searchParams.noResultUtilityForWhite) - 1;
  ...
  while(true) {
    ...
    if(bestChildIdx >= numChildrenFound) {
      ...
      if(searchParams.useGraphSearch)
        thread.graphHash = GraphHash::getGraphHash(/* thread.graphHash,*/ thread.history, thread.pla);
