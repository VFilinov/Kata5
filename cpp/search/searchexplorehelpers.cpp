...
double Search::getExploreSelectionValueOfChild(
  ...
  bool isDuringSearch, double maxChildWeight, bool countEdgeVisit, SearchThread* thread
) const {
  (void)parentUtility;
...
#ifndef QUANTIZED_OUTPUT
  const float* policyProbs = nnOutput?nnOutput->getPolicyProbsMaybeNoised():NULL;
#endif
  for(int i = 0; i<childrenCapacity; i++) {
    const SearchChildPointer& childPointer = children[i];
    ...
    if(nnPolicyProb < 0)
      continue;
    if(moveLoc==Board::PASS_LOC && searchParams.suppressPass)
      continue;
