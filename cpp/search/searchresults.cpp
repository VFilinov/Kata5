...
bool Search::getPlaySelectionValues(
  ...
  for(int i = 0; i < childrenCapacity; i++) {
    ...
    if(nnOutput) {
      ...
      policyProb = nnOutput->getPolicyProbMaybeNoised(getPos(moveLoc));
#else
      policyProb = policyProbs[getPos(moveLoc)];
#endif
    }
    if((suppressPass && moveLoc == Board::PASS_LOC) || policyProb <= 0) {
  ...
  if(&node == rootNode && numChildren > 0) {
    ...
    for(int i = 0; i<numChildren; i++) {
      ...
      const SearchNode* child = childPointer.getIfAllocated();
      Loc moveLoc = childPointer.getMoveLocRelaxed();
      if(suppressPass && moveLoc == Board::PASS_LOC && !rootHistory.maybePassMove(rootPla)) {
  ...
  if(maxValue <= 1e-50) {
    ...
    for(int i = 0; i<numChildren; i++) {
      ...
      double policyProb = policyProbs[movePos];
#endif
      playSelectionValues[i] = std::max(0.0,policyProb);
    }
    //Recompute max
    for(int i = 0; i<numChildren; i++) {
      if(playSelectionValues[i] > maxValue)
    ...
    if(humanProbs != NULL) {
      ...
      if(&node == rootNode && allowDirectPolicyMoves) {
        ...
        for(int movePos = 0; movePos<policySize; movePos++) {
          Loc moveLoc = NNPos::posToLoc(movePos,rootBoard.x_size,rootBoard.y_size,nnXLen,nnYLen);
#ifdef QUANTIZED_OUTPUT
          double humanProb = nnOutput->getPolicyProb(movePos);
#else
      ...
      for(int i = 0; i<childrenCapacity; i++) {
        ...
        Loc moveLoc = childPointer.getMoveLocRelaxed();
#ifdef QUANTIZED_OUTPUT
        double humanProb = nnOutput->getPolicyProb(getPos(moveLoc));
#else
...
#ifdef QUANTIZED_OUTPUT
          double humanProb = nnOutput->getPolicyProb(getPos(loc));
#else
...
Loc Search::getChosenMoveLoc() {
  ...
  // For prevent losing in renju - select best move
  if(rootHistory.maybePassMove(rootPla))
    temperature = 1.0e-4;
...
void Search::printPVForMove(ostream& out, const SearchNode* n, Loc move, int maxDepth) const {
  ...
  for(int i = 0; i<buf.size(); i++) {
    if(i > 0)
      out << " ";
    if(searchParams.logPVCoordinatesMode>=0)
      out << Location::toStringPsq(buf[i], rootBoard, searchParams.logPVCoordinatesMode);
    else
      out << Location::toString(buf[i], rootBoard);
  }
}
...
double Search::getShallowAverageShorttermWLError(const SearchNode* node) const {
  if(node == NULL)
    node = rootNode;
  if(node == NULL)
    return 0.0;
  ...
  getShallowAverageShorttermWLErrorHelper(
    node, graphPath, policyProbsBuf, minProp, desiredProp, wlError);
  return wlError;
}

void Search::getShallowAverageShorttermWLErrorHelper(
  const SearchNode* node,
  std::unordered_set<const SearchNode*>& graphPath,
  double policyProbsBuf[NNPos::MAX_NN_POLICY_SIZE],
  double minProp,
  double desiredProp,
  double& wlError) const {
  const NNOutput* nnOutput = node->getNNOutput();
  if(nnOutput == NULL) {
  ...
  // Find all children and compute weighting of the children based on their values
  {
    ...
    if(searchParams.useNoisePruning) {
      for(int i = 0; i < numChildren; i++)
#ifdef QUANTIZED_OUTPUT
        policyProbsBuf[i] = std::max(1e-30, (double)nnOutput->getPolicyProbMaybeNoised(getPos(statsBuf[i].prevMoveLoc)));
#else
        policyProbsBuf[i] = std::max(1e-30, (double)policyProbs[getPos(statsBuf[i].prevMoveLoc)]);
#endif
      totalChildWeight = pruneNoiseWeight(statsBuf, numChildren, totalChildWeight, policyProbsBuf);
  ...
  if(desiredPropFromChildren <= 0.0 || relativeChildrenWeightSum <= 0.0) {
    ...
    for(int i = 0; i < numChildren; i++) {
      ...
      getShallowAverageShorttermWLErrorHelper(
        child, graphPath, policyProbsBuf, minProp, desiredPropFromChild, wlError);
    }
  }
...
bool Search::getAnalysisJson(
  ...
  for(int i = 0; i < buf.size(); i++) {
    ...
    // scores (0 - for renju/gomoku)
    moveInfo["scoreMean"] = 0;
    moveInfo["scoreSelfplay"] = 0;
    moveInfo["scoreLead"] = 0;
    moveInfo["scoreStdev"] = 0;

    moveInfo["prior"] = Global::roundDynamic(data.policyPrior,OUTPUT_PRECISION);
    if(humanOutput != NULL)
#ifdef QUANTIZED_OUTPUT
      moveInfo["humanPrior"] = Global::roundDynamic(std::max(0.0,(double)humanOutput->getPolicyProbMaybeNoised(getPos(data.move))),OUTPUT_PRECISION);
#else
  ...
  // Stats for root position
  {
    ...
    // scores (0 - for renju/gomoku)
    rootInfo["scoreSelfplay"] = 0;
    rootInfo["scoreLead"] = 0;
    rootInfo["scoreStdev"] = 0;
    ...
    if(nnOutput != NULL) {
      ...
      rootInfo["rawLead"] = 0;
      rootInfo["rawScoreSelfplay"] = 0;
      rootInfo["rawScoreSelfplayStdev"] = 0;
    ...
    if(humanOutput != NULL) {
      ...
      for(int y = 0; y < board.y_size; y++) {
        for(int x = 0; x < board.x_size; x++) {
          int pos = NNPos::xyToPos(x, y, nnXLen);
#ifdef QUANTIZED_OUTPUT
          policy.push_back(Global::roundDynamic(humanOutput->getPolicyProbMaybeNoised(pos),OUTPUT_PRECISION));
#else
          policy.push_back(Global::roundDynamic(policyProbs[pos],OUTPUT_PRECISION));
#endif
        }
      ...
      int passPos = NNPos::locToPos(Board::PASS_LOC, board.x_size, nnXLen, nnYLen);
#ifdef QUANTIZED_OUTPUT
      policy.push_back(Global::roundDynamic(humanOutput->getPolicyProbMaybeNoised(passPos),OUTPUT_PRECISION));
#else
      policy.push_back(Global::roundDynamic(policyProbs[passPos],OUTPUT_PRECISION));
#endif
...
bool Search::getPrunedNodeValues(const SearchNode* nodePtr, ReportedSearchValues& values) const {
  ...
  double weightSum = 0.0;
  double weightSqSum = 0.0;
  for(int i = 0; i < numChildren /* childrenCapacity*/; i++) {
    const SearchChildPointer& childPointer = children[i];
    const SearchNode* child = childPointer.getIfAllocated();
    if(child == NULL)
      break;
