...
struct SearchParams {
  ...
  bool useVCFInput;
  bool useForbiddenInput;
  //bool disableUnnecessaryPass;
  bool suppressPass;
  ...
  // search. Deeper nodes may see history prior to the root of the search if searches were performed from earlier
  // positions and those gamestates were also reached by those earlier searches with the nn evals cached. This is true
  // even without tree reuse.
  ...
  */
  ...
  double minPlayoutsPerThread;  // If the number of playouts to perform per thread is smaller than this, cap the number
                                // of threads used.
