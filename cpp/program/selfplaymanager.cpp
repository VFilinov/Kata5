...
uint64_t SelfplayManager::getTotalNumMovesProcessed() const {
  std::lock_guard<std::mutex> lock(managerMutex);
  return totalNumMovesProcessed;
}

void SelfplayManager::setTotalNumMovesProcessed(uint64_t numMoves) {
  std::lock_guard<std::mutex> lock(managerMutex);
  totalNumMovesProcessed += numMoves;
}
...
void SelfplayManager::runDataWriteLoopImpl(ModelData* modelData) {
  ...
  while(true) {
    ...
    setTotalNumMovesProcessed(gameData->endHist.getCurrentTurnNumber());
