...
int Setup::computeDefaultEigenBackendThreads(int expectedConcurrentEvals, Logger& logger) {
  auto getNumCores = [&logger]() {
    ...
    if(numCores <= 0) {
      logger.write(
        "Could not determine number of cores on this machine, choosing eigen backend threads as if it were 8");
      numCores = 8;
    }
    return numCores;
  };
...
vector<SearchParams> Setup::loadParams(
  ...
  for(int i = 0; i<numBots; i++) {
    ...
    if(cfg.contains("minPlayoutsPerThread" + idxStr))
      params.minPlayoutsPerThread = cfg.getDouble("minPlayoutsPerThread" + idxStr, 0.0, 1.0e20);
    else if(cfg.contains("minPlayoutsPerThread"))
      params.minPlayoutsPerThread = cfg.getDouble("minPlayoutsPerThread", 0.0, 1.0e20);
    else
    ...
    if(cfg.contains("policyOptimism" + idxStr))
      params.policyOptimism = cfg.getDouble("policyOptimism" + idxStr, 0.0, 1.0);
    else if(cfg.contains("policyOptimism"))
      params.policyOptimism = cfg.getDouble("policyOptimism", 0.0, 1.0);
    else
    ...
    if(cfg.contains("rootPolicyOptimism" + idxStr))
      params.rootPolicyOptimism = cfg.getDouble("rootPolicyOptimism" + idxStr, 0.0, 1.0);
    else if(cfg.contains("rootPolicyOptimism"))
      params.rootPolicyOptimism = cfg.getDouble("rootPolicyOptimism", 0.0, 1.0);
    else
      params.rootPolicyOptimism =
        (setupFor != SETUP_FOR_DISTRIBUTED && setupFor != SETUP_FOR_OTHER) ? std::min(params.policyOptimism, 0.2) : 0.0;
    ...
    if(setupFor == SETUP_FOR_OTHER)
      params.suppressPass = false;
    else {
      if(cfg.contains("suppressPass" + idxStr))
        params.suppressPass = cfg.getBool("suppressPass" + idxStr);
      else if(cfg.contains("suppressPass"))
        params.suppressPass = cfg.getBool("suppressPass");
      else
        params.suppressPass = false;
    }
    ...
    /*
    if(cfg.contains("ignorePreRootHistory" + idxStr))
      params.ignorePreRootHistory = cfg.getBool("ignorePreRootHistory" + idxStr);
    else if(cfg.contains("ignorePreRootHistory"))
      params.ignorePreRootHistory = cfg.getBool("ignorePreRootHistory");
    else
      params.ignorePreRootHistory = (setupFor == SETUP_FOR_ANALYSIS ? Setup::DEFAULT_ANALYSIS_IGNORE_PRE_ROOT_HISTORY : false);
    if(cfg.contains("ignoreAllHistory" + idxStr))
      params.ignoreAllHistory = cfg.getBool("ignoreAllHistory" + idxStr);
    else if(cfg.contains("ignoreAllHistory"))
      params.ignoreAllHistory = cfg.getBool("ignoreAllHistory");
    else
    ...
    */
    ...
    if(cfg.contains("VCFProb" + idxStr))
      TRAINING_DATA_VCF_PROB = cfg.getDouble("VCFProb" + idxStr, 0.0, 1.0);
    else if(cfg.contains("VCFProb"))
      TRAINING_DATA_VCF_PROB = cfg.getDouble("VCFProb", 0.0, 1.0);
    else
      TRAINING_DATA_VCF_PROB = 0.9;
#else
    params.useVCFInput = false;
    TRAINING_DATA_VCF_PROB = 0;
#endif
    ...
    if(cfg.contains("ForbiddenProb" + idxStr))
      TRAINING_DATA_FORBIDDEN_FEATURE_PROB = cfg.getDouble("ForbiddenProb" + idxStr, 0.0, 1.0);
    else if(cfg.contains("ForbiddenProb"))
      TRAINING_DATA_FORBIDDEN_FEATURE_PROB = cfg.getDouble("ForbiddenProb", 0.0, 1.0);
    else
      TRAINING_DATA_FORBIDDEN_FEATURE_PROB = 0.5;
...
bool Setup::saveAutoPatternBonusData(
  const std::vector<Sgf::PositionSample>& genmoveSamples,
  ConfigParser& cfg,
  Logger& logger,
  Rand& rand) {
