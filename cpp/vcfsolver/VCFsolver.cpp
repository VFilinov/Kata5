...
int32_t VCFsolver::setBoard(const Board& b, uint8_t pla)
{
  ...
  if(rules.basicRule==Rules::BASICRULE_RENJU)
    forbiddenSide = (pla == C_BLACK) ? C_MY : C_OPP;//Если ход черных, то это 1, в противном случае это 2
  movenum = 0;
  bestmovenum = 10000;
  nodenum = 0;
  threeCount = 0;
      ...
      for (int x = 0; x < xsize; x++)
      {
        ...
        if (my % 6 == 0&& opp ==4)
        {
          ...
          else
          {
            ...
            if(anotherOppFourPos!= oppFourPos)result = -10000;//У соперника двойная четверка, но он не может вернуться напрямую, потому что он может быть напрямую связан с пятеркой.
          }
          continue;
        }
        if (my == 3 && opp%6 == 0)//Тройка
        {
...
int32_t VCFsolver::play(int x, int y, uint8_t pla, bool updateHash)
{
  ...
  //Переменные с суффиксом _forRenju гарантированно будут использоваться только в соответствии с правилом renju
  ...
  if (pla == C_MY)
  {
    ...
    //Нет необходимости рассматривать вопрос об отмене запрета в течение пяти дней подряд, поскольку результат был определен на один шаг вперед.
    //Метод проверки двойной четверки: Если найдены две последовательные пять точек, удваивайте четыре тогда и только тогда, когда расстояние между двумя точками равно[0,+-5],[+-5,0],[+-5,+-5]
    //Следующая раздача living four определенно выиграет, потому что нет запрета на пять раздач подряд.Конечно, уровень кода исключал, что соперник на шаг опережает четверку.
    //Если вы наберете больше пяти очков подряд, это должна быть двойная четверка запрещенных раздач.
    bool lifeFour_forRenju = false;
    bool isForbidden_forRenju = false;

    //Выполнение трех критериев: две или три последовательные тройки генерируются одновременно на одной и той же строке.После этого оценивается точка пересечения, является ли она запрещенной. Если 1 или 2 живые четыре точки, генерируемые линией, не запрещены, это означает, что это живая тройка.
    int8_t threeCountDir_forRenju[4] = {0, 0, 0, 0};//Количество новых 3 в каждом направлении, большее или равное 2, указывает на то, что может быть живая тройка
   // int16_t maybeLifeFourPoses[4][3] ;//Четыре балла за всю возможную последующую работу
    //Первое измерение - это 4 направления, первое число второго измерения - это количество живых четырех точек 1 или 2, второе и третье - это живые четыре точки

    movenum++;
    int32_t fourPos =-1;//Защитная точка стремительной четверки, образованная этим ходом
    bool winThisMove = false;//Запрещенной раздачи нет, дубль четыре
  //  cout << "s\n";
   ...
   //   cout << int(msc) << endl;
      ...
      {
        if (msc > 5 && msc % 6 == 5)isForbidden_forRenju = true;//Чанлянь
      }
      ...
      else if (msc == 4)
      {
        ...
        else
        {
          ...
          {
            //Проверьте, работает ли он в режиме реального времени.
            if (isPlaForbidden_forRenju)
            {
            ...
            {
                winThisMove = true;//Это запрещенная двойная четверка
            }
          }
        }
      }
    ...
    //Проверяйте только 33, потому что 44 давно уже проверено
    ...
    if (winThisMove)
    {
      result = 10000 - movenum - 1;//Двойная четверка или живая четверка
    ...
    if (fourPos==-1)//Эта шахматная комбинация - не бросок на четыре, попасть сюда можно, только временно заблокировав бросок противника на четыре.
    {
      if (oppFourPos == -1)
      {
 ...
 // int16_t maybeLifeFourPoses[4][3] ;//Четыре балла за всю возможную последующую работу
  //Первое измерение - это 4 направления, первое число второго измерения - это количество живых четырех точек 1 или 2, второе и третье - это живые четыре точки
    ...
    //todo renju:check if is forbidden
    oppFourPos = -1;//Бросок соперника на четыре очка напрямую фиксируется в oppFourPos
    ...
    auto addandcheck = [&](int t, int y, int x)
    {
      ...
      if (isPlaForbidden_forRenju)
      {
        if (osc > 5 && osc % 6 == 5)isForbidden_forRenju = true;//Чанлянь
      }
      if (mystonecount[t][y][x]%6 != 0 || osc < 3 || osc >5)return;//Никакой угрозы
      if (osc == 3)
      {
    ...
    if (isForbidden_forRenju)//Успешный арест
    {
      result = 10000 - movenum - 1;//Двойная четверка или живая четверка
      if (bestmovenum > movenum + 1)
      {
...
void VCFsolver::undo(int x, int y, int64_t oppFourPos1, uint64_t threeCount1, bool updateHash)
{
  ...
  oppFourPos = oppFourPos1;//Нет необходимости передавать эти два параметра, вы можете изменить их на месте, но я боюсь забыть
  //result = 0;
  auto pla = board[y][x];
  board[y][x] = 0;
  ...
  //Повторное использование кода в этом месте очень плохо. Если вы хотите его изменить, вам нужно многое изменить.
  if (pla == C_MY)
  {
    movenum--;
    //x
...
{
  //Посмотрите хэш-таблицу, прежде чем вычислять количество камней после удаления, здесь этого нет
  nodenum++;
  if (nodenum>= MAXNODE)
  {
    return -(movenum+1);
  ...
  if (movenum + 2 > maxMovenum)return -(movenum + 2);//Эта раздача определенно не сможет сделать пять подряд, тогда, по крайней мере, movenum+2 раздачи

  //Сначала сделайте резервную копию
  auto oppFourPos_old = oppFourPos;
  auto threeCount_old = threeCount;

  //Давайте посмотрим, равен ли предыдущий шаг соперника четырем.
  if (oppFourPos != -1)
  {
    int solutionPos = -1;
    int x = oppFourPos % sz, y = oppFourPos / sz;
    int32_t result=play(x,y, C_MY,true);
    if (resultNotSure(result) )//Это показывает, что ваша защита также набирает четыре очка, так что вы можете продолжать подсчитывать.Это не двойная четверка, так что требуется как минимум movenum+2 раздачи
    {
  ...
  //В противном случае это тот случай, когда соперник не бросился на четверых.
  int solutionPos = -1;
  ...
  for (int threeID = threeCount - 1; threeID >= 0; threeID--)
  {
    ...
    if (oppstonecount[t][y][x]%6 != 0 || mystonecount[t][y][x] != 3)continue;//Срок третьего сна истек
    ...
    if (bestresult >= 10000 - movenum - 2)break;//Если была найдена двойная четверка, то нет необходимости рассматривать другие пути
...
bool VCFsolver::isForbiddenMove(int y, int x,bool fiveForbidden)//јмІйЅыКЦ
{
  ...
  bool lifeFour = false;
    ...
    //Выполнение трех критериев: две или три последовательные тройки генерируются одновременно на одной и той же строке.После этого оценивается точка пересечения, является ли она запрещенной. Если 1 или 2 живые четыре точки, генерируемые линией, не запрещены, это означает, что это живая тройка.
    int8_t threeCountDir[4] = { 0, 0, 0, 0 };//Количество новых 3 в каждом направлении, большее или равное 2, указывает на то, что может быть живая тройка
    ...
    auto addandcheck = [&](int t, int y, int x)
    {
     ...
     // cout <<int(bsc)<<" ";
        if (bsc > 5 && bsc % 6 == 5)isForbidden = true;//і¤Б¬
    ...
    //Примечание: Эта функция вызывается, когда выносится третье суждение, fiveForbidden=true.Конечно, даже пять не является запрещенной раздачей, но эта функция будет вызвана только при оценке live three. Следующая раздача live three - live four. Если live four генерирует пять одновременно, это не live four. Рассмотрение даже пяти как запрещенной раздачи просто делает Суждение Live four верно.
...
bool VCFsolver::checkLife3(int y, int x, int t)//јмІйКЗ·сКЗ»оИэ
{
  //Предпосылка заключается в том, что он был отброшен и количество камней подсчитано
  auto p = forbiddenSide;
  auto stonecount = (p == C_MY) ? mystonecount : oppstonecount;
  auto ostonecount = (p == C_MY) ? oppstonecount:mystonecount  ;
  int16_t pos1 = -1, pos2 = -1;//Две или четыре точки, если есть только 0 или 1, заполните значением -1

  int ct=0;//Было несколько последовательных активных трех, ct=1 означает отсутствие активных четырех, ct = 2 означает одну активную четверку, а ct = 3 означает две активные четверки.
  int ctstart=-1;//Начиная с первых нескольких пятерок, происходит три последовательных появления.
  //В соответствии с t и stonecount найди работу в четыре часа
  ...
  case 0:
    ...
    for (int i = 0; i < 5; i++)
    {
      ...
      if (x1 < 0)break;
      if (x1 >= xsize - 4)continue;
      //Скопируйте следующий абзац 4 раза
      ...
      {
        if (ct >= 2)break;//Найдено непрерывное 3
        if (ct == 1)//Изолированный номер 3, игнорировать
        {
          ct = 0;
          ctstart = -1;
        }
    ...
    if (ct == 2)//Перейти к третьей прямой трансляции или другой заблокированной третьей прямой трансляции
    {
      //+xx+x+ или +x+xx+ или o+xxx++
      for (int i = 0; i < 4; i++)
      {
        int y1 = y ;
        int x1 = x - ctstart + i;
  ...
  case 2:
    ...
    else if (ct == 3)//Живите втроем подряд
    {
      //+xxx+
