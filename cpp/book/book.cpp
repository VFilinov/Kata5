...
BookParams Book::getParams() const {
  return params;
}
void Book::setParams(const BookParams& p) {
  params = p;
}
...
// Returns false at the end of the whole search if f never aborts. Also this particular function supports
// trying to follow low-cost parents first.
bool Book::reverseDepthFirstSearchWithMoves(
  const BookNode* initialNode,
  bool preferLowCostParents,
  const std::function<Book::DFSAction(const vector<const BookNode*>&, const vector<Loc>&)>& f) const {
...
)
{
  ...
  int numItems;
  {
    ...
    for(auto iter = node->moves.begin(); iter != node->moves.end(); ++iter) {
      ...
      sortValuesBuf.push_back(getSortingValue(
        plaFactor,
        vals.winLossValue,
        iter->second.rawPolicy));
  ...
  std::sort(sortIdxBuf.begin(), sortIdxBuf.end(), [&](const int& idx0, const int& idx1) {
...
void Book::recomputeNodeCost(BookNode* node) {
  ...
  for(auto& locAndBookMove: node->moves) {
    ...
    double costFromWL = ucbWinLossLoss * params.costPerUCBWinLossLoss +
    ...
    // cout << "Setting child " << (int)locAndBookMove.first << " cost from root, parentMinCostFromRoot " <<
    // node->minCostFromRoot
  ...
  if(!node->canExpand) {
    ...
    // If the proposed expansion is significantly better in utility than most expanded moves, the penalty should not be
    // as large.
    ...
    for(auto& otherLocAndBookMove: node->moves) {
      ...
      double otherChildUtility = getUtility(otherChild->recursiveValues);
      double gainOverOtherChild =
        (node->pla == P_WHITE) ? notInBookUtility - otherChildUtility : otherChildUtility - notInBookUtility;
    ...
    // cout << "Expansion stats " << ucbWinLossLoss << " " << rawPolicy << endl;
    ...
    //      << " costFromUCB " << costFromUCB
    //      << " cost due to log policy (" << rawPolicy << ") " << (-boostedLogRawPolicy * costPerLogPolicy)
    //      << " moves expanded cost " << (movesExpanded * costPerMovesExpanded + movesExpanded * movesExpanded *
  ...
  for(auto& locAndBookMove: node->moves) {
    ...
    if(bestOtherCostFromRoot < locAndBookMove.second.costFromRoot) {
      ...
      //      << " becomes " << locAndBookMove.second.costFromRoot + 0.50 * (bestOtherCostFromRoot -
      //      locAndBookMove.second.costFromRoot) << endl;
  ...
  }
  {
    ...
    if(bestOtherCostFromRoot - node->minCostFromRoot < node->thisNodeExpansionCost) {
      ...
      //      << " becomes " << node->thisNodeExpansionCost + 0.50 * (bestOtherCostFromRoot - node->minCostFromRoot -
      //      node->thisNodeExpansionCost) << endl;
  ...
  }
  {
    ...
    // For the final node expansion cost, sharp score discrepancy beyond 1 point is not capped, to encourage expanding
    // the search to better resolve the difference, since sharp score can have some weird outliers.
    double movesExpanded = (double)node->moves.size();
    ...
    double bonus =
      params.bonusPerWinLossError * winLossError + params.bonusPerExcessUnexpandedPolicy * excessUnexpandedPolicy;
    ...
    const double BEST_WINLOSS_OFFSET = 0.02;
    {
      ...
      if(anyOtherWinLossFound && winLoss > bestOtherWinLoss) {
        double visitsFactor = 0.5 * (std::min(1.0, sqrt(bestOtherVisits / std::max(1.0, params.visitsScale))) +
    ...
    if(node->moves.size() >= 2) {
      ...
      if(bestWinLossChild != NULL && bestWinLossChild->recursiveValues.visits <= params.maxVisitsForReExpansion) {
        ...
        for(auto& locAndBookMoveOther: node->moves) {
          ...
          if(otherChild != bestWinLossChild) {
            double winLossOtherThisPerspective = (node->pla == P_WHITE) ? otherChild->recursiveValues.winLossValue
                                                                        : -otherChild->recursiveValues.winLossValue;
          ...
          for(auto& locAndBookMove: node->moves) {
            ...
            if(child == bestWinLossChild) {
              locAndBookMove.second.costFromRoot -=
                0.75 * params.bonusPerUnexpandedBestWinLoss *
                (bestWinLossThisPerspective - bestOtherWinLossThisPerspective + BEST_WINLOSS_OFFSET) * visitsFactor;
              break;
            }
          }
        }
      ...
      auto behindInVisitsBonus = [&](double childWinLoss, double adjustedVisits) {
        ...
        for(auto& otherLocAndBookMove: node->moves) {
          ...
          double gainOverOtherChild =
            (node->pla == P_WHITE) ? childWinLoss - otherChildWinLoss : otherChildWinLoss - childWinLoss;
          if(gainOverOtherChild <= -params.policyBoostSoftUtilityScale)
            continue;
          double thisBonus = log10(otherVisits / (30.0 * adjustedVisits)) -
                             0.5 * log10(std::max(adjustedVisits, params.visitsScaleLeaves) / params.visitsScaleLeaves);
          if(gainOverOtherChild < 0.0) {
            double factor =
              (gainOverOtherChild + params.policyBoostSoftUtilityScale) / (params.policyBoostSoftUtilityScale + 1e-10);
        ...
        // Also scale down by how far we are from the best, and scale down by if we are close to losing so nothing
        // matters
        double gainOverBest = (node->pla == P_WHITE) ? childWinLoss - bestWinLoss : bestWinLoss - childWinLoss;
        // Just in case
      ...
      //     const BookNode* child = get(locAndBookMove.second.hash);
      //     double childWinLoss = child->recursiveValues.winLossValue;
      //     double childWinLossPersp = (node->pla == P_WHITE) ? child->recursiveValues.winLossValue :
      //     -child->recursiveValues.winLossValue; double adjustedVisits = child->recursiveValues.adjustedVisits; double
      //     childBonus = behindInVisitsBonus(childWinLoss,adjustedVisits); cout << "child " << childWinLossPersp << " "
      //     << adjustedVisits << " " << childBonus << endl;
      //   }
      //   {
      //     double childWinLoss = node->thisValuesNotInBook.winLossValue;
      //     double childWinLossPersp = (node->pla == P_WHITE) ? node->thisValuesNotInBook.winLossValue :
      //     -node->thisValuesNotInBook.winLossValue; double adjustedVisits = node->thisValuesNotInBook.visits; double
      //     childBonus = behindInVisitsBonus(childWinLoss,adjustedVisits); cout << "xpand " << childWinLossPersp << " "
      //     << adjustedVisits << " " << childBonus << endl;
      //   }
      // }
    }
  ...
  for(auto& locAndBookMove: node->moves) {
    locAndBookMove.second.costFromRoot =
      node->minCostFromRoot + (locAndBookMove.second.costFromRoot - node->minCostFromRoot) * depthFromRootFactor;
  }
  {
    ...
    if(node->moves.size() < requiredBranch) {
      ...
      if(childEnoughVisitsCount < requiredBranch) {
        ...
        for(auto& locAndBookMove: node->moves) {
          ...
          sortValuesBuf.push_back(getSortingValue(plaFactor, vals.winLossValue, locAndBookMove.second.rawPolicy));
      ...
      for(int x = 0; x<board.x_size; x++) {
        ...
        if(!child.isNull()) {
          ...
          if(child.recursiveValues().visits >= htmlMinVisits) {
            ...
            linkSymmetriesStr +=
              Global::intToString(x + y * board.x_size) + ":" + Global::intToString(child.symmetryOfNode) + ",";
          }
        }
      }
    }
...
void Book::saveToFile(const string& fileName) const {
...

  {
    ...
    // paramsDump["adjustedVisitsWLScale"] = params.adjustedVisitsWLScale; // not used
...
Book* Book::loadFromFile(const std::string& fileName) {
  ...
  try {
    ...
    std::unique_ptr<Book> book;
    {
      ...
      //bookParams.adjustedVisitsWLScale = params.contains("adjustedVisitsWLScale") ? params["adjustedVisitsWLScale"].get<double>() : 0.05; // не используется
    ...
    while(getline(in,line)) {
      ...
      // So force it true on old versions. Parameter changes can alter whether a node is expandable or
      // not (e.g. whether it's considered done given all its visits) But it's not much harm to set a node as
      // non-expandable, since except for error cases this only happens when a node has explored all possible legal
      // moves, in which case we might as well not use this node either for reexpansions, if reexpansions can only
      // target child nodes, that's fine.
      if(book->bookVersion >= 2)
