...
struct StringError : public std::exception {
  std::string message;
  std::string message_detail; // Added by VF
  ...
  StringError(const std::string& m)
    :exception(),message(m)
  {}
  StringError(const char* m, const char* d) : exception(), message(m), message_detail(d) {}
  StringError(const std::string& m, const std::string& d) : exception(), message(m), message_detail(d) {}
  StringError(const std::string& m, const char* d) : exception(), message(m), message_detail(d) {}

  const char* what() const throw() final {
    if(message_detail.empty())
      return message.c_str();
    else
      return message_detail.c_str();
  }
...
struct IOError : public StringError { IOError(const char* msg):StringError(msg) {}; IOError(const std::string& msg):StringError(msg) {}; 
                                            IOError(const char* msg, const char* detail) : StringError(msg, detail){};
                                            IOError(const std::string& msg, const std::string& detail) : StringError(msg, detail){};
                                            IOError(const std::string& msg, const char* detail) : StringError(msg, detail){};
};
