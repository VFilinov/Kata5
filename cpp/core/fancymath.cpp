...
void FancyMath::runTests() {
...

  {
    ...
    x = (1.0 + sqrt(5)) / 2.0;
    y = evaluateContinuedFraction([](int n) noexcept { (void)n; return 1.0; }, [](int n) { (void)n; return 1.0; }, 1e-15, 1000);
    APPROX_EQ(x,y,1e-14);

    x = sqrt(2);
    y = evaluateContinuedFraction([](int n) noexcept { (void)n; return 1.0; }, [](int n) { return n == 0 ? 1.0 : 2.0; }, 1e-15, 1000);
    APPROX_EQ(x,y,1e-14);

    x = exp(1);
    y = evaluateContinuedFraction([](int n) noexcept { (void)n; return 1.0; }, [](int n) { return n == 0 ? 2.0 : n%3 == 2 ? (double)((n+1)/3*2) : 1.0; }, 1e-15, 1000);
    APPROX_EQ(x,y,1e-14);

    x = PI;
    y = evaluateContinuedFraction([](int n) noexcept { return (n*2-1)*(n*2-1); }, [](int n) { return n == 0 ? 3.0 : 6.0; }, 1e-15, 10000);
    APPROX_EQ(x,y,1e-10);
    y = evaluateContinuedFraction([](int n) noexcept { return n == 1 ? 4 : ((n-1)*(n-1)*4-1); }, [](int n) { return n == 0 ? 2.0 : n == 1 ? 3.0 : 4.0; }, 1e-15, 10000);
    APPROX_EQ(x,y,1e-8);
    y = evaluateContinuedFraction([](int n) noexcept { return n == 1 ? 2 : n*(n-1); }, [](int n) { return n == 0 ? 2.0 : 1.0; }, 1e-15, 10000);
    APPROX_EQ(x,y,1e-3);
  }
