...
SimpleDate::SimpleDate() : year(1970), month(1), day(1) {}

SimpleDate::SimpleDate(int y, int m, int d) : year(y), month(m), day(d) {
  if(!isValid(year, month, day))
    throw StringError(
      "SimpleDate: Invalid year month day:" + Global::intToString(year) + "," + Global::intToString(month) + "," +
      Global::intToString(day));
}

SimpleDate::SimpleDate(const SimpleDate& other) : year(other.year), month(other.month), day(other.day) {}
...
static const int CUMULATIVE_DAYS_UNTIL_MONTH[14] = {0, 0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365};
...
void DateTime::runTests() {
  ...
  for(int i = 0; i < 100000; i++) {
    ...
    testAssert(((((((SimpleDate() + d1) + d2) - d1) + d3) - d2) - d3) == SimpleDate());
