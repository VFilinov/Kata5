...
void FinishedGameData::printDebug(ostream& out) const {
  ...
  for(int i = 0; i < whiteQValueTargetsByTurn.size(); i++) {
    ...
    for(int j = 0; j < target.size(); j++)
      out << Location::toString(target[j].loc, startBoard) << " " << target[j].winLoss << " " << target[j].visits << " ";
    out << endl;
  }
...
static int16_t clampToRadius32000(float x, Rand& rand) {
  ...
  if(lambda == 0.0f)
    ...
    return (int16_t)(rand.nextBool(lambda) ? high : low);
}

static void fillQValueTarget(
  const vector<QValueTargetMove>& whiteQValueTargets,
  Player nextPlayer,
  int policySize,
  int dataXLen,
  int dataYLen,
  ...
  for(size_t i = 0; i < size; i++) {
    ...
    cPosTarget[pos + policySize] = clampToRadius32000(0/* score * 60.0f*/, rand);
  ...
  if(actualGameEndHist.isGameFinished && actualGameEndHist.isNoResult) {
    ...
    for(int y = 0; y < board.y_size; y++) {
      for(int x = 0; x < board.x_size; x++) {
        ...
        rowOwnership[pos + posArea * 4] = clampToRadius120(scoring*120.0, rand);
      }
    }
    */
  }
