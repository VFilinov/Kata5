...
static bool testAllConfigs(
  ...
  if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
    ...
    message.clear();
    message << "Testing " << configs.size() << " different configs";
    ...
    if(accums.bad) {
      if(verboseErrors) {
        if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
          ...
          message.clear();
          message << "Tuning " << i << "/" << configs.size() << " failed: " << getErrorMessage(accums.badErr);
          logger->write(message.str());
        }
      ...
      if(i == 0) {
        ...
        if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
          ...
          message.clear();
          message << "WARNING: Reference implementation failed: " << getErrorMessage(accums.badErr);
          logger->write(message.str());
        }
      }
    }
        ...
        if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
          out << "Tuning "
              << (!verboseTuner       ? ""
                  : score > bestScore ? "* "
                                      : "  ")
              << i << "/" << configs.size() << (i == 0 ? " (reference)" : "") << " Calls/sec " << kernelsPerSecond
              << " ErrorProp " << errorProp << " " << getDesc(configs[i]) << endl;
        else {
          message.clear();
          message << "Tuning "
              << (!verboseTuner       ? ""
                  : score > bestScore ? "* "
                                      : "  ")
              << i << "/" << configs.size() << (i == 0 ? " (reference)" : "") << " Calls/sec " << kernelsPerSecond
              << " ErrorProp " << errorProp << " " << getDesc(configs[i]);
          logger->write(message.str());
        }
  ...
  if(!anythingGoodYet) {
    if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
      ...
      message.clear();
      message << "ERROR: Could not find any configuration that worked";
      logger->write(message.str());
    }
    return false;
  }
...
static void tuneXGemmDirect(
  ...
  if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
    ...
    message.clear();
    message << "Tuning xGemmDirect for 1x1 convolutions and matrix mult";
    logger->write(message.str());
  }
...
static bool tuneXGemm(
  ...
  if(useFP16Storage) {
    if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
      ...
      message.clear();
      message << "Tuning xGemm for convolutions - trying with FP16 storage";
      logger->write(message.str());
    }
  }
  else {
    if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
      ...
      message.clear();
      message << "Tuning xGemm for convolutions";
      logger->write(message.str());
    }
  }
...
static bool tuneXGemm16(
  ...
  if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
    ...
    message.clear();
    message << "Tuning xGemm16 for convolutions";
    logger->write(message.str());
  }
...
static bool tuneHGemmWmma(
  ...
  if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
    ...
    message.clear();
    message << "Tuning hGemmWmma for convolutions";
    logger->write(message.str());
  }
...
static bool tuneHGemmWmmaNCHW(
  ...
  if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
    ...
    message.clear();
    message << "Tuning hGemmWmmaNCHW for 1x1 convolutions";
    logger->write(message.str());
  }
...
static void tuneTransform(
  ...
  if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
    ...
    message.clear();
    message << "Tuning winograd transform for convolutions";
    logger->write(message.str());
  }
...
static void tuneUntransform(
  ...
  if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
    ...
    message.clear();
    message << "Tuning winograd untransform for convolutions";
    logger->write(message.str());
  }
...
static void tuneGPool(
  ...
  if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
    ...
    message.clear();
    message << "Tuning global pooling strides";
    logger->write(message.str());
  }
...
void OpenCLTuner::tune(
  ...
  if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
    ...
    message.clear();
    message << "Beginning GPU tuning for " << device->info.name << " modelVersion " << modelInfo.modelVersion << " channels " << modelInfo.trunkNumChannels;
    logger->write(message.str());
  }
  ...
  if(winograd3x3TileSize == 2) {
    if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
      ...
      message.clear();
      message << "Setting winograd3x3TileSize = 2";
      logger->write(message.str());
    }
    ...
    if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
      ...
      message.clear();
      message << "Setting winograd3x3TileSize = 4";
      logger->write(message.str());
    }
  ...
  if(!currentConfig.isValid()) {
    if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
      ...
      message.clear();
      message << "Loaded a config but the config was invalid, starting tuning from basic config";
      logger->write(message.str());
    }
...

  {
    ...
    if(!shouldTestFP16) {
      if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
        ...
        message.clear();
        message << "Not enabling FP16 for anything since FP16 disabled";
        logger->write(message.str());
      }
    }
    else {
      ...
      if(shouldTestFP16TensorCores) {
        {
          ...
          if(!suc) {
            if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
              ...
              message.clear();
              message << "FP16 tensor core tuning failed, assuming no FP16 tensor core support";
              logger->write(message.str());
            }
          }
            ...
            if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
              ...
              message.clear();
              message << "FP16 tensor cores not significantly faster, not enabling";
              logger->write(message.str());
            }
          }
          else {
            ...
            if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
              ...
              message.clear();
              message << "Enabling FP16 tensor cores due to better performance";
              logger->write(message.str());
            }
          }
        }
        {
          ...
          if(!suc) {
            if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
              ...
              message.clear();
              message << "FP16 tensor core tuning failed for 1x1 convs";
              logger->write(message.str());
            }
          ...
          // If we're using tensor cores normally, AND they're fast enough, then use them for 1x1 convs.
            ...
            if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
              ...
              message.clear();
              message << "FP16 tensor cores enabled for 1x1 convs";
              logger->write(message.str());
            }
            currentConfig = result16;
            ...
            if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
              ...
              message.clear();
              message << "FP16 tensor cores not enabled for 1x1 convs";
              logger->write(message.str());
            }
      ...
      if(shouldTestFP16Compute) {
        ...
        if(!suc) {
          if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
            ...
            message.clear();
            message << "FP16 compute tuning failed, assuming no FP16 compute support";
            logger->write(message.str());
          }
          ...
          if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
            ...
            message.clear();
            message << "FP16 compute not significantly faster, not enabling";
            logger->write(message.str());
          }
        }
          ...
          if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
            out << "FP16 compute not significantly faster than tensor cores, using it generally but using tensor cores "
                   "for convs" << endl;
          else {
            message.clear();
            message << "FP16 compute not significantly faster than tensor cores, using it generally but using tensor cores "
                   "for convs";
            logger->write(message.str());
          }
        }
          ...
          if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
            ...
            message.clear();
            message << "Enabling FP16 compute due to better performance";
            logger->write(message.str());
          }
        }
      }
      ...
      if(shouldTestFP16Storage) {
        ...
        if(!suc) {
          if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
            ...
            message.clear();
            message << "FP16 storage tuning failed, assuming no FP16 storage support";
            logger->write(message.str());
          }
        }
        else if(bestKernelsPerSecond16 / FP16_REQUIRED_SPEEDUP < bestKernelsPerSecond) {
          ...
          if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
            ...
            message.clear();
            message << "FP16 storage not significantly faster, not enabling on its own";
            logger->write(message.str());
          }
        }
        else {
          ...
          if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
            ...
            message.clear();
            message << "Enabling FP16 storage due to better performance";
            logger->write(message.str());
          }
        }
      }
  ...
  if(currentConfig.shouldUseFP16TensorCores) {
    if(!logger->isLoggingToStdout() && !logger->isLoggingToStderr())
      ...
      message.clear();
      message << "Using FP16 tensor cores!";
      logger->write(message.str());
    }
  }
