...
struct Buffers {
  ...
  Buffers(CudaHandles* cudaHandles, const Model& m, const ScratchBuffers& scratch) {
    ...
    if(NNModelVersion::getSupportedVersion(m.modelVersion, NNModelVersion::Q_VALUE_PREDICTIONS))
      testAssert(m.policyHead->p2Channels == 4);
    else if(NNModelVersion::getSupportedVersion(m.modelVersion, NNModelVersion::DOUBLE_POLICY_CHANELLS))
...
void NeuralNet::getOutput(
  ...
  for(int row = 0; row < batchSize; row++) {
    ...
    if(numPolicyChannels == 2 || (numPolicyChannels == 4 && NNModelVersion::getSupportedVersion(modelVersion, NNModelVersion::Q_VALUE_PREDICTIONS))) {
