...
ActivationLayerDesc::ActivationLayerDesc(istream& in, int modelVersion) {
  in >> name;
  if(NNModelVersion::getSupportedVersion(modelVersion, NNModelVersion::SELECT_ACTIVATION)) {
    string kind;
    in >> kind;
...
TrunkDesc::TrunkDesc(istream& in, int vrsn, bool binaryFloats, int metaEncVersion) {
  ...
  if(NNModelVersion::getSupportedVersion(modelVersion, NNModelVersion::NONLINEARITY_PASS_POLICY)) {
    int unused;
    in >> unused;
    in >> unused;
    in >> unused;
...
PolicyHeadDesc::PolicyHeadDesc(istream& in, int vrsn, bool binaryFloats) {
  in >> name;
  modelVersion = vrsn;
  policyOutChannels = NNModelVersion::getSupportedVersion(modelVersion, NNModelVersion::DOUBLE_POLICY_CHANELLS) + 1;

  if(NNModelVersion::getSupportedVersion(modelVersion, NNModelVersion::Q_VALUE_PREDICTIONS))
    policyOutChannels = 4;
  else if(NNModelVersion::getSupportedVersion(modelVersion, NNModelVersion::DOUBLE_POLICY_CHANELLS))
    policyOutChannels = 2;
  else
    policyOutChannels = 1;
...
ModelDesc::ModelDesc(istream& in, const string& sha256_, bool binaryFloats) {
  ...
  if(NNModelVersion::getSupportedVersion(modelVersion, NNModelVersion::IS_MULTIPLIER)) {
...
Rules ModelDesc::getSupportedRules(const Rules& desiredRules, bool& supported) const {
  ...
  supported = true;
  // May be modyfing rules
  /*
  supported = false;
  int mode = NNModelVersion::getSupportedVersion(modelVersion, NNModelVersion::TYPE_VCN_RULE);
  if(mode == NNModelVersion::RULE_NOT_VCN) {
    supported = (rules.firstPassWin == false) && (rules.VCNRule == Rules::VCNRULE_NOVC) && (rules.maxMoves == 0);
  } else if(mode == NNModelVersion::RULE_IS_VCN) {
